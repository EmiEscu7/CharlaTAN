from RASAComponents.ScrumAssistant.tour.chain.criterion import (
    AndCriterion,
    NotCriterion,
...
from RASAComponents.ScrumAssistant.tour.chain.node import (
    DefaultNode,
    Node,
    NodeActionListen,
    NodeNext,
...
def chain_builder(last_node: DefaultNode) -> Node:
    ...
    chain = NodeGet(
        ...
        AndCriterion(
            ...
            AndCriterion(
                EqualIntent("not_understand"), NotCriterion(EqualEntity(None))
            )
        )
    )
    chain = NodeRepeat(
        ...
        OrCriterion(
            ...
            AndCriterion(
                ...
                AndCriterion(EqualIntent("not_understand"), EqualEntity(None))
            )
        )
    )
    chain = NodeExample(
    ...
    chain = NodeAsk(
        chain, OrCriterion(
            EqualAction("utter_ask"), EqualAction("utter_ask_good")
        )
    )
    chain = NodeResponse(chain,
                         EqualPenultimateEvent("utter_cross_examine_topic"))
    return chain
