...
from . import util as tour_util
from .criterion import Criterion
from ..conversation.abstract_flow import ConversationFlow
from ..visitor.ask_visitor import AskVisitor
from ..visitor.example_visitor import ExampleVisitor
from ..visitor.topic_visitor import TopicVisitor
...
class NodeNext(Node):
    ...
    def next(self, it: ConversationFlow, tracker: DialogueStateTracker) -> str:
        if self._criterion.check(tracker):
            next_utter = it.next()
            tour_util.move_to_location(next_utter)
            tour_util.move_task(next_utter)
            return next_utter
        else:
            return self._node.next(it, tracker)
