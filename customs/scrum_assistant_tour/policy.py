...
from . import util
from .chain import util as chain_util, node
from .learning_styles_detection import Dimension, LearningStyleDetector
...
class AssistantPolicy(Policy):
    ...
    def __init__(
        ...
        conversation_flows = util.create_conversation_flows(
            "." + os.path.sep + "customs" + os.path.sep + "scrum_assistant_tour" + os.path.sep + "info" + os.path.sep + "flow.json")
    ...
    def predict_action_probabilities(
        ...
        if not self._set_timer_after_utter:
            print("LA ACCION A REALIZAR POR EL SA ES ------------> " + str(action))
